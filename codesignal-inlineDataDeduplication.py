#https://app.codesignal.com/company-challenges/purestorage/MfRt23G9R5xjp6jzh
...
class StorageArray:
    ...
    def write(self, data):
        ...
        """
        addr = None
        if data in self.dedup_lookaside.keys():
    ...
    def read(self, addr):
        ...
        if addr < len(self.data_storage) and self.data_storage[addr] in self.dedup_lookaside.keys():
    ...
    def delete(self, addr):
        ...
        """
        if addr < len(self.data_storage) and self.data_storage[addr] in self.dedup_lookaside.keys():
            self.dedup_lookaside[self.data_storage[addr]].refs -= 1
            ans = self.data_storage[addr]
            if self.dedup_lookaside[self.data_storage[addr]].refs == 0:
                self.dedup_lookaside.pop(self.data_storage[addr])
                self.data_storage[addr] = ""
            return ans
